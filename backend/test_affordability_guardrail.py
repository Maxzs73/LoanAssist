...
class TestApplyAffordabilityGuardrail:
...
class TestLoanServiceAffordabilityIntegration:
    ...
    def test_unaffordable_case_rejected(self):
        ...
        # The affordability reason must be present
        assert any(
            'EMI' in r and '50%' in r
            for r in result.get('rejection_reasons', [])
        ), f"Affordability reason missing from: {result['rejection_reasons']}"

        # Improvement suggestion must be present
        assert any(
            'smaller loan' in s.lower() or 'longer tenure' in s.lower()
            for s in result.get('improvement_suggestions', [])
        ), f"Affordability suggestion missing from: {result['improvement_suggestions']}"
